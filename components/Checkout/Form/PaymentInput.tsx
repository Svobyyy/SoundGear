...
export default function PaymentInput({ labelName, error }: Props) {
  error = true;
  return (
    <label
      htmlFor={`${labelName.toLowerCase()}`}
      className={`${error ? "border-red" : ""} group relative mt-[18px] flex w-full cursor-pointer select-none items-center gap-4 rounded-lg border border-gray py-[19px] pl-[18px] text-[14px] font-bold text-black outline-none transition-all hover:border-orange focus:border-orange has-[:checked]:border-orange`}
    >
      <input
        type="radio"
